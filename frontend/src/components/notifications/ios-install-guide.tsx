...
export function IosInstallGuide({ compact = false }: { compact?: boolean }) {
  ...
  return (
    ...
    >
      <p className="m-0 text-than font-medium text-text-main">{t("title")}</p>
      {!compact && (
        <p className="mb-0 mt-1 text-than leading-relaxed text-text-muted">{t("why")}</p>
      )}
      <ol className="mb-0 mt-2 list-decimal space-y-1 pl-5 text-than leading-relaxed text-text-muted">
        {steps.map((step) => (
